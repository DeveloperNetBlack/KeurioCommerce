...
namespace Keurio.Infrastructure.DB.SQLSERVER.Repositories.UbigeoRepositories
{
    internal class UbigeoListByUbigeoClassRepository : IUbigeoListByUbigeoClassRepository
    {
        ...
        {
            var listadoUbigeo = new List<UbigeoListByUbigeoClassResponseDto>();
            var Connection = await TransactionAccessor.GetOrOpenConnectionAsync(ConnectionString, CancellationToken);
            using (SqlCommand Command = new SqlCommand())
            {
                Command.CommandText = "Security.spUbigeoListByUbigeoClass";
                ...
                using (DataReader = await Command.ExecuteReaderAsync(CancellationToken))
                {
                    if (DataReader.HasRows)
                    {
                        ...
                        {
                            var listado = new UbigeoListByUbigeoClassResponseDto()
                            {
